...
- The README's "automatic (confirmable) detection of translation mods" claim is now accurate (see above). Its batch "Update all" claim still isn't — updates remain per-mod only, no bulk action exists. Left as-is deliberately (aspirational copy under an explicit "status: in development" badge) — revisit if it gets built, or if the gap needs closing sooner.
